...
import org.springboot.sample.dao.PermissionDao;
import org.springboot.sample.dao.RoleDao;
...
public class MyShiroRealm extends AuthorizingRealm{
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        if(user!=null){
            ...
            //用户的角色集合
            info.setRoles(user.getRolesName(roleList));
//            List<Role> roleList=user.getRoleList();
            for (Role role : roleList) {
            	List<Permission> perlist = permissionDao.getPermissionList(role.getId());
                info.addStringPermissions(role.getPermissionsName(perlist));
            }
            // 或者按下面这样添加
